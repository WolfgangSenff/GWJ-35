...
func _physics_process(delta: float) -> void:
    ...
    if ledge_cast_colliding and not above_cast_colliding and not _has_grabbed_ledge:
        _play_left_right_anim("LedgeGrab")
        _velocity += Vector2(80 if _last_animation.ends_with("Right") else -80, -340)
